...
describe('giving job registration', () => {
  it('registers event processing and scheduled reconciliation with focused inputs', async () => {
    ...
    const reconcile = vi.fn().mockResolvedValue({ events: 2, eventFailures: 1, verifications: 1, verificationFailures: 0, continuations: 1, continuationFailures: 1, cancellations: 1, cancellationFailures: 0 })
    ...
    expect(tasks[1].outputSchema?.map((field) => 'name' in field ? field.name : null)).toEqual(['events', 'eventFailures', 'verifications', 'verificationFailures', 'continuations', 'continuationFailures', 'cancellations', 'cancellationFailures'])
