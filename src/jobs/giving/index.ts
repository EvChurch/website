...
interface GivingJobRunners {
  ...
  reconcile(payload: Payload): Promise<{ events: number; eventFailures: number; verifications: number; verificationFailures: number; continuations: number; continuationFailures: number; cancellations: number; cancellationFailures: number; draftsDeleted: number; recurringSchedules: number; recurringFailures: number }>
}
...
export function createGivingJobConfigs(runners: GivingJobRunners) {
  ...
  const reconcile: TaskConfig<{ input: Record<string, never>; output: { events: number; eventFailures: number; verifications: number; verificationFailures: number; continuations: number; continuationFailures: number; cancellations: number; cancellationFailures: number; draftsDeleted: number; recurringSchedules: number; recurringFailures: number } }> = {
    ...
    outputSchema: [
      ...
      { name: 'recurringSchedules', type: 'number', required: true },
      { name: 'recurringFailures', type: 'number', required: true },
    ],
