...
function keyRing(): SigningKey[] {
  ...
  if (!value) {
    if (process.env.NODE_ENV !== 'production') {
      return [{ kid: 'development', secret: Buffer.alloc(32, 7) }]
    }
    throw new Error('ROCK_CONNECTION_CONTEXT_KEYS is required')
  }
...
function validAttribute(
  value: unknown,
): value is RockConnectionContextAttribute {
  if (
    !isRecord(value) ||
    !exactKeys(value, [
      'attributeGuid',
    ...
    return false
  if (
    !GUID_PATTERN.test(String(value.attributeGuid)) ||
    !GUID_PATTERN.test(String(value.fieldTypeGuid))
  )
    return false
  if (
    ...
    !isRecord(value.configurationValues)
  )
    return false
...
function parseContext(value: unknown, now: number): RockConnectionContext {
  ...
  if (
    ...
    !GUID_PATTERN.test(String(value.pageGuid)) ||
    !GUID_PATTERN.test(String(value.blockGuid)) ||
    !GUID_PATTERN.test(String(value.opportunityGuid)) ||
    !GUID_PATTERN.test(String(value.sessionGuid)) ||
    !GUID_PATTERN.test(String(value.interactionGuid)) ||
    typeof value.nonce !== 'string' ||
    !/^[A-Za-z0-9_-]{24,128}$/.test(value.nonce) ||
    !Array.isArray(value.campuses) ||
    value.campuses.length > 100 ||
    !value.campuses.every(
      (campus) => typeof campus === 'string' && /^\d{1,10}$/.test(campus),
    ) ||
    !(
      value.selectedCampusId === null ||
      Number.isSafeInteger(value.selectedCampusId)
    ) ||
    typeof value.displayHomePhone !== 'boolean' ||
    typeof value.displayMobilePhone !== 'boolean' ||
    !Array.isArray(value.attributes) ||
    value.attributes.length > 100 ||
    !value.attributes.every(validAttribute) ||
    !Number.isSafeInteger(issuedAt) ||
    !Number.isSafeInteger(expiresAt) ||
    (expiresAt as number) <= now ||
    (issuedAt as number) > now + CLOCK_SKEW_MS ||
    (expiresAt as number) - (issuedAt as number) > MAX_LIFETIME_MS
  )
    invalid()
...
export function createRockConnectionContextToken(
  context: RockConnectionContext,
  now = Date.now(),
): string {
...
export function verifyRockConnectionContextToken(
  token: string,
  now = Date.now(),
  ...
  if (
    ...
    header.alg !== 'HS256' ||
    header.typ !== 'ROCK-CONNECTION-CONTEXT' ||
  ...
  const expected = Buffer.from(
    sign(`${encodedHeader}.${encodedPayload}`, key.secret),
  )
