...
function context(overrides: Partial<RockConnectionContext> = {}): RockConnectionContext {
  return {
    version: 1,
...
describe('Rock Connection context tokens', () => {
  beforeEach(() => {
    vi.stubEnv('ROCK_CONNECTION_CONTEXT_KEYS', `current:${secret},previous:${previous}`)
  })
  ...
  it('signs with the first key and verifies during a bounded rotation overlap', () => {
    ...
    vi.stubEnv('ROCK_CONNECTION_CONTEXT_KEYS', `next:${previous},current:${secret}`)
    expect(verifyRockConnectionContextToken(token, now)).toEqual(context())
  })

  it('rejects tampered, expired, and future tokens', () => {
    const token = createRockConnectionContextToken(context(), now)
    expect(() => verifyRockConnectionContextToken(`${token.slice(0, -1)}x`, now)).toThrow('Invalid connection context')

    const expired = createRockConnectionContextToken(
      context({ issuedAt: now - 400_000, expiresAt: now - 1 }),
      now - 300_000,
    )
    expect(() => verifyRockConnectionContextToken(expired, now)).toThrow('Invalid connection context')

    const future = createRockConnectionContextToken(
      context({ issuedAt: now + 31_000, expiresAt: now + 100_000 }),
      now + 31_000,
    )
    expect(() => verifyRockConnectionContextToken(future, now)).toThrow('Invalid connection context')
  })

  it('refuses to create cross-protocol or cross-audience tokens', () => {
    expect(() => createRockConnectionContextToken({ ...context(), purpose: 'rock-form' as never }, now)).toThrow('Invalid connection context')
    expect(() => createRockConnectionContextToken({ ...context(), audience: 'other' as never }, now)).toThrow('Invalid connection context')
  })
  ...
  it('rejects unknown fields, algorithm confusion, unknown keys, and oversized input', () => {
    ...
    for (const variant of variants) {
      expect(() => verifyRockConnectionContextToken(variant, now)).toThrow('Invalid connection context')
    }
  })
})
