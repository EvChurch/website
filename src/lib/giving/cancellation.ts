  ...
  return {
    ...
    begin(input) {
      return transaction(pool, async (client) => {
        ...
        const operation = await client.query(`INSERT INTO giving_provider_operations(context_key,environment,synthetic,e2e_run_id,checkout_id,schedule_id,actor_id,reason,provider,action,logical_version,request_digest,correlation_key,request_id,idempotency_key,status) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'blinkpay','blinkpay.cancel-schedule',$9,$10,$11,$12,$13,'prepared') RETURNING id`, [row.context_key,row.environment,row.synthetic,row.e2e_run_id,row.checkout_id,input.scheduleId,input.actorId,input.reason,version,requestDigest,`${row.correlation_key}:cancel:${version}`,input.requestId,input.idempotencyKey])
