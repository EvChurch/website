...
import { GivingFrequencyPreview } from '../GivingAnswerTrail'

export function AmountStep({ value, error, showFrequencyPreview, onContinue }: { value: number | null; error?: string; showFrequencyPreview: boolean; onContinue: (minor: number | null) => void }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); onContinue(canContinue ? amountMinor : null) }} className="space-y-4">
    ...
    {canContinue && showFrequencyPreview && <GivingFrequencyPreview />}
  </form>
}
