...
export function AmountStep({ value, transactionFeeMinor, error, onContinue }: { value: number | null; transactionFeeMinor: number; error?: string; onContinue: (minor: number | null) => void }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); onContinue(canContinue ? amountMinor : null) }} className="space-y-4">
    ...
    {transactionFeeMinor > 0 && <p id="giving-amount-fee" className="text-sm text-dark-grey">+${(transactionFeeMinor / 100).toFixed(2)} transaction fee.</p>}
