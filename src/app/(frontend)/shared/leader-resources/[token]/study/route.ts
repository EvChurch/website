...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const asset = await getPublicLeaderResourceAsset(token, 'study')
  if (!asset || asset.kind !== 'study') return memberMediaNotFound()
  try {
    const file = await fetchMemberRockFile(asset.guid)
    return file ? memberMediaResponse(file.body, file.contentType, {
      'Content-Disposition': `inline; filename="${asset.name.replace(/[^A-Za-z0-9._ -]/gu, '').slice(0, 120) || 'member-study'}"`,
