...
ALTER TABLE blinkpay_webhook_events ADD CONSTRAINT blinkpay_webhook_events_provenance_valid CHECK((environment='production' AND NOT synthetic AND context_key='production') OR (environment='sandbox' AND synthetic AND context_key='sandbox') OR (environment='sandbox' AND synthetic AND context_key='sandbox:unmatched' AND (status='quarantined' OR (provider_reference_type='payment' AND status IN ('pending','processing','retry','dead')))) OR (environment='production' AND NOT synthetic AND context_key='production:unmatched' AND (status='quarantined' OR (provider_reference_type='payment' AND status IN ('pending','processing','retry','dead')))));
DO $$ BEGIN
...
ALTER TABLE blinkpay_webhook_events DROP CONSTRAINT IF EXISTS blinkpay_webhook_events_provenance_valid;
ALTER TABLE blinkpay_webhook_events ADD CONSTRAINT blinkpay_webhook_events_provenance_valid CHECK((environment='production' AND NOT synthetic AND context_key='production') OR (environment='sandbox' AND synthetic AND context_key='sandbox'));
ALTER TABLE blinkpay_webhook_events DROP CONSTRAINT IF EXISTS blinkpay_webhook_events_attempt_count_valid;
ALTER TABLE blinkpay_webhook_events DROP CONSTRAINT IF EXISTS blinkpay_webhook_events_duplicate_count_valid;
ALTER TABLE blinkpay_webhook_events DROP CONSTRAINT IF EXISTS blinkpay_webhook_events_conflict_count_valid;
