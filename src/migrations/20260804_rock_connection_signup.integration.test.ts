...
import { ROCK_CONNECTION_SIGNUP_UP_SQL } from './20260804_rock_connection_signup'
...
describe.skipIf(!databaseUrl)('Rock connection signup migration on PostgreSQL', () => {
  let client: Client
  ...
  async function resetFixture({ mismatched = false } = {}): Promise<void> {
    ...
    await client.query(`
      ...
      INSERT INTO _pages_v_blocks_form_embed VALUES
        (200, 10, 'layout', 5, 'centered', '${oldWorkflowGuid}'),
        (201, 11, 'layout', 5, 'centered', '${oldWorkflowGuid}');
    `)
  }
  ...
  it('converts only reviewed Newish rows and enforces source identity', async () => {
    ...
    const versions = await client.query(`
      ...
      WHERE source_type::text = 'connectionOpportunity'
        ...
        AND rock_connection_block_guid = '495cda8e-60fe-4f77-a452-932b460fb44c'
    `)
    ...
    await expect(client.query(`
      ...
      VALUES
        (102, 2, 'layout', 2, 'centered', 'workflow', '${oldWorkflowGuid}', '495cda8e-60fe-4f77-a452-932b460fb44c')
