...
import { EXPLAINING_CHRISTIANITY_CONNECTION_BLOCK_GUID } from '@/seed/explaining-christianity-form'
...
describe('Rock connection signup migration', () => {
  ...
  it('expands and backfills both form table families without rewriting workflow GUIDs', () => {
    ...
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
    ...
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      'ADD COLUMN IF NOT EXISTS "rock_connection_block_guid"',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toMatch(
      /SET "source_type" = 'workflow'[\s\S]+WHERE "source_type" IS NULL/g,
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).not.toMatch(
      /SET\s+"rock_workflow_guid"\s*=\s*(?!NULL)/,
    )
  })
  ...
  it('targets only the reviewed Newish candidate in live and version rows', () => {
    ...
    const newishCorrectionStatements = correctionStatements?.filter((statement) =>
      statement.includes("= 'newish'"),
    )
    expect(newishCorrectionStatements).toHaveLength(2)
    expect(newishCorrectionStatements?.[0]).toContain(
      'lower(p."slug") = \'newish\'',
    )
    expect(newishCorrectionStatements?.[1]).toContain(
      'lower(v."version_slug") = \'newish\'',
    )
  })

  it('converts the reviewed Explaining Christianity workflow to its matching connection signup', () => {
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      'lower(p."slug") = \'explaining-christianity\'',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      'lower(v."version_slug") = \'explaining-christianity\'',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
  ...
  it('allows empty databases but aborts duplicate and mismatched Newish candidates', () => {
    ...
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).not.toContain(
  ...
  it('asserts valid discriminators then adds equivalent constraints and lookup indexes', () => {
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      'invalid live form embed source rows',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      'invalid version form embed source rows',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      'pages_blocks_form_embed_source_identity_check',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      '_pages_v_blocks_form_embed_source_identity_check',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      'pages_blocks_form_embed_connection_lookup_idx',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      '_pages_v_blocks_form_embed_connection_lookup_idx',
    )
    expect(
      ROCK_CONNECTION_SIGNUP_UP_SQL.match(/"rock_workflow_guid" IS NOT NULL/g),
    ).toHaveLength(4)
    expect(
      ROCK_CONNECTION_SIGNUP_UP_SQL.match(
        /"rock_connection_block_guid" IS NOT NULL/g,
      ),
    ).toHaveLength(4)
  })
  ...
  it('creates nonce and rate tables that match U2 with bounded expiry cleanup', () => {
    ...
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
      'rock_connection_signup_cleanup_expired',
    )
    expect(ROCK_CONNECTION_SIGNUP_UP_SQL).toContain(
  ...
  it('refuses down before any DDL when either table family has Connection rows', () => {
    const guard = ROCK_CONNECTION_SIGNUP_DOWN_SQL.indexOf(
      'Cannot roll back Rock Connection Signup',
    )
    const firstDdl = ROCK_CONNECTION_SIGNUP_DOWN_SQL.indexOf('DROP TRIGGER')
    expect(guard).toBeGreaterThan(-1)
    expect(firstDdl).toBeGreaterThan(guard)
    expect(ROCK_CONNECTION_SIGNUP_DOWN_SQL.slice(0, firstDdl)).toContain(
  ...
  it('keeps Workflow-only down lossless and makes a forced database assertion fail the unit', async () => {
    expect(ROCK_CONNECTION_SIGNUP_DOWN_SQL).not.toContain(
      'DROP COLUMN "rock_workflow_guid"',
    )
    const execute = vi
      .fn()
      .mockRejectedValue(new Error('forced assertion failure'))
    await expect(up(migrationArgs(execute).args)).rejects.toThrow(
